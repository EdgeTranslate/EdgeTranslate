...
translationButtonIFrame.id = "edge-translate-button";
translationButtonIFrame.style.backgroundColor = "white"; // programatically set style to compatible with the extension 'Dark Reader'
const ImageData =
...
function showButton(event) {
    ...
    if (XPosition <= 0 || XPosition + translationButtonIFrame.clientWidth > window.innerWidth)
        XPosition = event.x - XBias - translationButtonIFrame.clientWidth;
    if (YPosition <= 0 || YPosition + translationButtonIFrame.clientHeight > window.innerHeight)
...
function scrollHandler() {
    if (HasButtonShown) {
        ...
        translationButtonIFrame.style.left = `${originPositionX + distanceX}px`;
        translationButtonIFrame.style.top = `${originPositionY + distanceY}px`;
    }
}
