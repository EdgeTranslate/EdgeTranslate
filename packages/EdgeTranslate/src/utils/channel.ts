...
class Channel {
        ...
        Browser.runtime.onMessage.addListener(
            ((message: string, sender: Runtime.MessageSender) => {
                ...
                switch (parsed.type) {
                    ...
                    default:
                        console.error(`Unknown message type: ${(parsed as { type: string }).type}`);
                        break;
                }
                return;
            }).bind(this)
    ...
    public provide(
        serviceName: string,
        serveFunction: (params: unknown, sender: Runtime.MessageSender) => Promise<unknown>
    ): () => void {
     ...
     */
    public request(serviceName: string, params: Record<string, unknown> | unknown) {
    ...
    public requestToTab(
        ...
        if (!Browser.tabs || !Browser.tabs.sendMessage || tabId === undefined)
     ...
     */
    public on(event: string, handler: (detail: unknown, sender: Runtime.MessageSender) => unknown) {
        const emitHandler = ((data: { detail: unknown; sender: Runtime.MessageSender }) => {
     ...
     * @param detail - event detail
     */
    public emitToTabs(tabIds: number | number[] | undefined, event: string, detail: unknown) {
        if (!Browser.tabs || !Browser.tabs.sendMessage || tabIds === undefined)
            return Promise.reject("Can not send message to tabs in current context!");
