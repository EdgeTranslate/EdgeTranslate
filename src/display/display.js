...
function addEventListener() {
    ...
    frameDocument
        .getElementById("icon-tuding-fix")
        .addEventListener("click", fixOn);
    frameDocument
        .getElementById("icon-tuding-full")
        .addEventListener("click", fixOff);
...
function clickListener(event) {
    ...
    if (!isChildNode(node, frame)) {
        var boundary =
            popupPosition === "left"
                ? frame.offsetLeft + frame.clientWidth
                : frame.offsetLeft; // 根据侧边栏的位置确定拖拽的起点
...
function drag(event) {
    if (mousedown) {
        var moveLength =
            popupPosition === "left"
                ? -(originX - event.screenX)
                : originX - event.screenX; // 根据侧边栏的位置确定移动距离的增减
        frame.style.width = moveLength + originWidth + "px";
        document.body.style.width =
            originOriginWidth - originWidth - moveLength + "px";
    }
}
